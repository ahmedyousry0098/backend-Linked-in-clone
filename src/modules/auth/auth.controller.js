...
export const register = async (req, res, next) => {
    const {email, password, username, position} = req.body
    if (await UserModel.findOne({email})) {
        return next(new ResponseError('Email Already Exist', 409))
    }
    const user = new UserModel({username, email: email.toLowerCase(), password, position})
...
export const confirmEmail = async (req, res, next) => {
    ...
    return res.status(202).redirect('http://localhost:5500/authentication/login/login.html')
}
...
export const logIn = async (req, res, next) => {
    ...
    if (!user.isConfirmed) {
        return next(new ResponseError('Please Confirm Your Email First', 403))
    }
